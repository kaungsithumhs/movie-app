        ...
        <div className="bg-white shadow-md border-gray-100  border sm:rounded-3xl p-3 flex justify-around h-full">
          ...
          <div className="w-[50%] p-1">
            <div className="justify-start">
              ...
              <h2 className="mt-3">
                {profileData?.data?.birthday.split("-").join(".")}
              </h2>
              <p className="mt-3">
                {profileData?.data?.biography
                  ? turncate(profileData?.data?.biography, 700)
                  : "Loading..."}
              </p>
              <button
                onClick={() => navigate(-1)}
                className="absolute bottom-[110px] right-20 text-blue-500 hover:text-red-500 cursor-pointer"
              >
                &laquo; Go Back
              </button>
            </div>
